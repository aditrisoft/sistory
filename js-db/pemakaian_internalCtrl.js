adiapp.controller('pemakaian_internalCtrl', function($scope,$http) {
  ...
  $scope.hapusData = function(kode){
    ...
    if (r==false){return;}
    $http.post(link_database+"list-pemakaian_internal-cud.php",{
      'cud':'hapus',
      'nokwit':kode
...
//    $scope.isi=[];
  ...
  $scope.tampilJumlah = function(){
    $scope.animation_image=true;
    $http.get(link_database+'list-pemakaian_internal.php?cari='+$scope.txtcari).success(function(data,status,header,config){
      $scope.jumx=data;
  ...
  function loadlagi(){
    ...
    //Posisi scroll mentok ke bawah
      if((posisiS+layarS) >= (tinggiS-1)){
        ...
        angular.element(document.getElementById('pemakaian_internalCtrl')).scope().tampilProduk();
      }
  }
