adiapp.controller('nozzleCtrl', function($scope,$http) {
  ...
  $scope.hapusData = function(kode){
    ...
    if (r==false){return;}
    $http.post(link_database+"list-nozzle-cud.php",{
      'cud':'hapus',
      'idnozzle':kode
...
//      alert('Sukses Menyimpan!');
  ...
  $scope.detailData = function(kode){
    ...
    $http.post(link_database+"list-nozzle-cud.php",{
      ...
      $scope.cud=data;
      $scope.txtnozzle=$scope.cud[0].nozzle;
...
//    $scope.isi=[];
  ...
  $scope.tampilJumlah = function(){
    $scope.animation_image=true;
    $http.get(link_database+'list-nozzle.php?cari='+$scope.txtcari).success(function(data,status,header,config){
      $scope.jumx=data;
  ...
  function loadlagi(){
    ...
    //Posisi scroll mentok ke bawah
      if((posisiS+layarS) >= (tinggiS-1)){
        ...
        angular.element(document.getElementById('nozzleCtrl')).scope().tampilProduk();
      }
  }
