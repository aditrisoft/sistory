adiapp.controller('produkCtrl', function($scope,$http) {
...
//      alert('Hapus Sukses!');
  ...
  $http.post(link_database+"list-produk-cud.php",{
      ...
      'produk_nama':$scope.txtproduk_nama,
      'harga_jual':$scope.txtharga_jual
    }).then(function(response){
      $scope.tampilData();
      $scope.bersihData();
//      alert('Sukses Menyimpan!');
  ...
  $scope.detailData = function(kode){
    ...
    $http.post(link_database+"list-produk-cud.php",{
      ...
      $scope.txtharga_jual=parseFloat($scope.cud[0].harga_jual);      
      $('#myModal').modal('show');
     },function(error){
      alert('Gagal Menyimpan!');
    });
...
//    $scope.isi=[];
  ...
  $scope.tampilJumlah = function(){
    $scope.animation_image=true;
    $http.get(link_database+'list-produk.php?cari='+$scope.txtcari).success(function(data,status,header,config){
      $scope.jumx=data;
