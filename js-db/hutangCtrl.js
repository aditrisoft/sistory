adiapp.controller('hutangCtrl', function($scope,$http) {
  ...
  $scope.ProsesBayar = function(){
    ...
    if (r==false){return;}
    $http.post(link_database+"list-hutang-cud.php",{
        'cud':'cicil',
...
//    $scope.isi=[];
  ...
  $scope.tampilJumlah = function(){
    $scope.animation_image=true;
    $http.get(link_database+'list-hutang.php?tglawal='+iTgl($scope.txttglawal)+'&tglakhir='+iTgl($scope.txttglakhir)).success(function(data,status,header,config){
      $scope.jumx=data;
      ...
      $scope.animation_image=true;
        $scope.isi=data;
          ...
          for (i=0;i<parseFloat($scope.isi.length);i++){
            $scope.totalhutang=parseFloat($scope.totalhutang)+parseFloat($scope.isi[0].nominal);
  ...
  function loadlagi(){
    ...
    //Posisi scroll mentok ke bawah
      if((posisiS+layarS) >= (tinggiS-1)){
        ...
        angular.element(document.getElementById('hutangCtrl')).scope().tampilProduk();
      }
  }
