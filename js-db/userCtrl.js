adiapp.controller('userCtrl', function($scope,$http) {
  ...
  $scope.hapusData = function(kode){
    ...
    if (r==false){return;}
    $http.post(link_database+"list-user-cud.php",{
      'cud':'hapus',
      'usernama':kode
...
//      alert('Hapus Sukses!');
  ...
  $http.post(link_database+"list-user-cud.php",{
      'cud':'input',
      'usernama':$scope.txtusernama,      
      'pass':$scope.txtpass,
      'levelx':$scope.txtlevelx,
      'nama':$scope.txtnama,
      'nik':$scope.txtnik
    }).then(function(response){
      $scope.tampilData();
...
//      alert('Sukses Menyimpan!');
  ...
  $scope.detailData = function(kode){
    ...
    $http.post(link_database+"list-user-cud.php",{
      ...
      $scope.cud=data;
      $scope.txtusernama=$scope.cud[0].usernama;
      $scope.txtpass=$scope.cud[0].pass;
      $scope.txtalamat=$scope.cud[0].alamat;      
      $scope.txtnama=$scope.cud[0].nama;
      $scope.txtnik=$scope.cud[0].nik;
      $scope.txtlevelx=$scope.cud[0].levelx;
      $('#myModal').modal('show');
     },function(error){
      alert('Gagal Menyimpan!');
    });
...
//    $scope.isi=[];
  ...
  $scope.tampilJumlah = function(){
    $scope.animation_image=true;
    $http.get(link_database+'list-user.php?cari='+$scope.txtcari).success(function(data,status,header,config){
      $scope.jumx=data;
  ...
  function loadlagi(){
    ...
    //Posisi scroll mentok ke bawah
      if((posisiS+layarS) >= (tinggiS-1)){
        ...
        angular.element(document.getElementById('userCtrl')).scope().tampilProduk();
      }
  }
