adiapp.controller('trbankCtrl', function($scope,$http) {
  ...
  $scope.hapusData = function(kode){
    ...
    if (r==false){return;}
    $http.post(link_database+"list-trbank-cud.php",{
      'cud':'hapus',
      'id':kode
...
//      alert('Hapus Sukses!');
  ...
  $scope.SimpanData = function(){
    ...
    $http.post(link_database+"list-trbank-cud.php",{
        ...
        'noperkbank':$scope.txtnoperkbank,
        'tgl':$scope.txttgl,
        'usernama':'admin'
      }).success(function(data, status, headers, config){
        $scope.tampilData();
...
//    $scope.isi=[];
  ...
  $scope.tampilJumlah = function(){
    $scope.animation_image=true;
    $http.get(link_database+'list-trbank.php?cari='+$scope.txtcari).success(function(data,status,header,config){
      $scope.jumx=data;
  ...
  function loadlagi(){
    ...
    //Posisi scroll mentok ke bawah
      if((posisiS+layarS) >= (tinggiS-1)){
        ...
        angular.element(document.getElementById('trbankCtrl')).scope().tampilProduk();
      }
  }
