adiapp.controller('pelangganCtrl', function($scope,$http) {
...
//    $scope.isi=[];
  ...
  $scope.tampilJumlah = function(){
    $scope.animation_image=true;
    $http.get(link_database+'list-pelanggan.php?cari='+$scope.txtcari).success(function(data,status,header,config){
      $scope.jumx=data;
  ...
  function loadlagi(){
    ...
    //Posisi scroll mentok ke bawah
      if((posisiS+layarS) >= (tinggiS-1)){
        ...
        angular.element(document.getElementById('pelangganCtrl')).scope().tampilProduk();
      }
  }
