adiapp.controller('noperkCtrl', function($scope,$http) {
...
//      alert('Sukses Menyimpan!');
  ...
  $scope.detailData = function(kode){
    $scope.animation_image_modal=true;
    $scope.TmbUpdate=false;
    $('#myModal').modal('show');
    $scope.dskode=true;
    ...
    $http.post(link_database+"list-noperk-cud.php",{
      ...
      $scope.animation_image_modal=false;
...
//    $scope.isi=[];
  ...
  $scope.tampilJumlah = function(){
    $scope.animation_image=true;
    $http.get(link_database+'list-noperk.php?cari='+$scope.txtcari).success(function(data,status,header,config){
      $scope.jumx=data;
  ...
  function loadlagi(){
    ...
    //Posisi scroll mentok ke bawah
      if((posisiS+layarS) >= (tinggiS-1)){
        ...
        angular.element(document.getElementById('noperkCtrl')).scope().tampilProduk();
      }
  }
