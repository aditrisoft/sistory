adiapp.controller("lab03Ctrl", function($scope, $http) {

	$scope.judullap="Pembelian Per Produk";
	...
	$scope.tampilLap = function(){
		$scope.animation_image=true;
		$http.get('api-server/list-lab03.php?tglawal='+iTgl($scope.txttglawal)+'&tglakhir='+iTgl($scope.txttglakhir)+'&cud=lap').success(function(data,status,header,config){
			$scope.listx=data;
	...
	$scope.tampilTotalLap = function(){
		$scope.animation_image=true;
		$http.get('api-server/list-lab03.php?tglawal='+iTgl($scope.txttglawal)+'&tglakhir='+iTgl($scope.txttglakhir)+'&cud=tot').success(function(data,status,header,config){
			$scope.totx=data;
